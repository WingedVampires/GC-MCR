...
//		HashSet<AbstractNode> otherVariableNodes = new HashSet<AbstractNode>();	
	...
	public Vector<String> generateSchedule(StringBuilder causalConstraint, long gid, Long wgid, long gid_prefix)
	{
        ...
        Pair<Boolean, StringBuilder> isNeedSchedule = constraintsFilter.doFilter_with_expression(CONS_ASSERT_PO, CONS_ASSERT_VALID, causalConstraint);

        if(isNeedSchedule.getKey()){
            id.incrementAndGet();
            ConstraintsSolving task = new ConstraintsSolving(config,id.get());
            ConstraintsFilter.totalConstraintsCounts ++;
            System.out.println("进行了第"+ ConstraintsFilter.totalConstraintsCounts +"次约束求解......");
            StringBuilder miniConstraint = new StringBuilder("").append(CONS_ASSERT_PO).append(CONS_ASSERT_VALID).append(isNeedSchedule.getValue());
            declareVariables(miniConstraint);
            String CONS_SETLOGIC = "(set-logic QF_IDL)\n";
            task.sendMessage(CONS_SETLOGIC + CONS_DECLARE + miniConstraint + CONS_GETMODEL,makeVariable(gid),makeVariable(wgid), makeVariable(gid_prefix),reachEngine, causalConstraint.toString(), config);

            return task.schedule;
        }else {
            ConstraintsFilter.filtedConstraintsCounts ++;
            System.out.println("过滤掉了"+ ConstraintsFilter.filtedConstraintsCounts +"约束求解......" );
